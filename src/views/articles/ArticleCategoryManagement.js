...
const ArticleCategoryManagement = () => {
  ...
  useEffect(() => {
    ...
    const getCategories = async () => {
      const response = await articleApi.fetchCategories({ params })
    ...
    const columns1 = [
      {
        ...
        title: '',
        render: (_, { article_category_id }) => (
          <CFormCheck
            checked={checkedState[article_category_id]}
            id="checkboxNoLabel"
            value={article_category_id}
            aria-label="..."
            onChange={() => handleIdCBChange(article_category_id)}
          />
        ),
        renderColumn: () => (
          <CFormCheck
      ...
      },
      {
        ...
        title: 'ID',
        sortable: true,
        render: (_, { article_category_id }) => <>#{article_category_id}</>,
      },
      {
        key: 'category_name',
        title: 'Danh mục',
            ...
            <CDropdown variant="btn-group" className="dropdown-btn-group">
              ...
              <CDropdownMenu>
                <NavLink className="text-nodecorate" to="#">
                  <CDropdownItem>Sửa danh mục</CDropdownItem>
                </NavLink>
              </CDropdownMenu>
            </CDropdown>
          </div>
  ...
  useEffect(() => {
    const states = {}
    data.forEach((d) => {
      states[d.article_category_id] = false
    })
  ...
  const deleteCategories = async (params) => {
    try {
      await articleApi.deleteCategories({ params })
      ...
      <CRow>
        <CCol xs={12}>
          <CCard className="mb-4 card-custom">
            <CCardHeader>
              <div className="header-title-custom">Quản lý danh mục bài viết</div>
                  ...
                  <CCol lg={6} md={6} sm={8}>
                    <CRow className="justify-content-end gap-2">
                      <CCol xl={5} lg={5} md={6} sm={6} xs={12}>
                        <NavLink to="../article-categories/create">
