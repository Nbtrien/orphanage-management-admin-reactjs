...
const InformationPageManagement = () => {
  ...
  useEffect(() => {
    ...
    const getInformationPages = async () => {
      const response = await contentApi.fetchInformationPage({ params })
    ...
    const searchInformationPages = async () => {
      try {
    ...
    const columns1 = [
      {
        key: 'page_type_id',
        title: '',
        render: (_, { page_type_id }) => (
          <CFormCheck
            checked={checkedState[page_type_id]}
            id="checkboxNoLabel"
            value={page_type_id}
            aria-label="..."
            onChange={() => handleIdCBChange(page_type_id)}
          />
        ),
        renderColumn: () => (
          <CFormCheck
      ...
      },
      {
        ...
        title: 'ID',
        sortable: true,
        render: (_, { page_type_id }) => <>#{page_type_id}</>,
      },
      {
        key: 'page_type',
        title: 'Trang',
            ...
            <CDropdown variant="btn-group" className="dropdown-btn-group">
              ...
              <CDropdownMenu>
                <NavLink
                  ...
                  to={`../content/information-page/create?pageType=${page_type_id}`}
                >
                  <CDropdownItem>Cập nhật nội dung</CDropdownItem>
                </NavLink>
              </CDropdownMenu>
            </CDropdown>
          </div>
  ...
  const deleteData = async (params) => {
    try {
      await contentApi.deleteFaqs({ params })
      ...
      <CRow>
        <CCol xs={12}>
          <CCard className="mb-4 card-custom">
            <CCardHeader>
              <div className="header-title-custom">Quản lý trang thông tin</div>
                ...
                <CRow className="justify-content-between">
                  <CCol lg={6} md={6} sm={4} xs={12} className="mb-2">
                    <CRow className="justify-content-start gap-2">
                      <CCol xl={4} lg={5} md={5} sm={10} xs={12}></CCol>
                    </CRow>
                  </CCol>
                  <CCol lg={6} md={6} sm={8}>
                    <CRow className="justify-content-end gap-2">
                      <CCol xl={5} lg={5} md={6} sm={6} xs={12}></CCol>
...
export default InformationPageManagement
