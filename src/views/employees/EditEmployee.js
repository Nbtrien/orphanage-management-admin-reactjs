...
const EditEmployee = () => {
  ...
  useEffect(() => {
    const getEmployee = async () => {
      try {
        const response = await employeeApi.getEmployeeUpdateInfo(id)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await employeeApi.updateEmployeeInfo(id, JSON.stringify(data))
      setLoadingModalVisible(false)
  ...
  useEffect(() => {
      ...
      !successModalVisible &&
      (window.location.href = 'http://localhost:3000/admin/employees/' + id)
          ...
          <CCard className="mb-4">
            <CCardBody>
              <div className="form-header-custom mb-4">
                <h4>Cập nhật thông tin nhân viên</h4>
              </div>
              <div className="mb-3">
